...
#### 4.Use `table()` to see the Cayley table of the particular group
```{r}
table(x)
```

#### 5.Use can use + operation to do the arithmetic for the group, and based on the number we use, it will return the number + 1 row of the Cayley table. When the number is greater than the total row number, it just starts from the first row. There are three different way to do the arithmetic:
